...
. Откройте конфигурационный файл `C:\Program Files\PostgresPro\12\data\postgresql.conf` (используйте свой путь установки {pgsql}).
...
+
include::new-admin-console@install-linux:ROOT:partial$excerpts.adoc[tags=pangolinagent]
+
. Если для подключения к {pgsql} планируется использовать аутентификацию Windows, необходима <<win-auth,дополнительная настройка СУБД>>.
. Для пользователя, от имени которого будет выполняться инициализация БД {dv} и последующее управление, необходимы права `SUPERUSER`.
...
.. Для работы xref:6.1@platform:ROOT:requirements-server-account.adoc[сервера {dv}], сервиса xref:6.1@platform:ROOT:requirements-full-text-account.adoc[полнотекстового поиска] и xref:6.1@platform:ROOT:requirements-file-service-account.adoc[файлового сервиса] может использоваться одна учетная запись.
+
...
+
include::new-admin-console@install-linux:ROOT:partial$excerpts.adoc[tags=pgagent]
+
